...
xss = idx.(xs, ys')
yss = idx.(zs', xs)
surface!(ax, xss, p1.(xs, ys'), yss; colormap = [myred], alpha = 0.5, transparency = true)
ts = range(-1, 1, 2)
l(t) = [1+t, 1, -2t] 
points = Point3.(l.(ts))
...
save("img/derivadas-funciones-varias-variables/derivada-parcial-x." * graphictype, fig, pt_per_unit = 1)
fig