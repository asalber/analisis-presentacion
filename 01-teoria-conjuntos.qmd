...
:::{#exm-cota-superior}
Para el conjunto $B=\{3, 4, 5\}\subseteq \mathbb{N}$, $5$ es una cota superior. El conjunto de todas sus cotas superiores es $\operatorname{CS}(B)=\{x:x\geq 5\}$.

Para el intervalo $(0,1)\subseteq \mathbb{R}$, cualquier número real $x\geq 1$ es una cota superior, por lo que $\operatorname{CS}([0,1)) = \{x:x\geq 1\}$.
:::
...
El conjunto de las cotas inferiores de $[0,1)\subseteq \mathbb{R}$ es $\operatorname{CI}([0,1)) = \{x:x\leq 0\}$. 
:::
...
:::{#def-maximo-conjunto}
## Máximo
Dado un conjunto con una relación de orden parcial $(A,\preceq)$, se dice que un elemento $m\in A$ es un _máximo_ de $A$, y se denota $\max(A)$, si cualquier otro elemento de $A$ es mayor o igual que él según la relación de orden, es decir, 
...
:::{#def-minimo-conjunto}
## Mínimo
Dado un conjunto con una relación de orden parcial $(A,\preceq)$, se dice que un elemento $m\in A$ es un _mínimo_ de $A$, y se denota $\min(A)$, si y cualquier otro elemento de $A$ es menor o igual que él según la relación de orden, es decir, 
...
:::{#exm-supremo}
El supremo del conjunto $B=\{3, 4, 5\}\subseteq \mathbb{N}$, es $5$ ya que es el mínimo del conjunto de sus cotas superiores $\operatorname{CS}(B)=\{x:x\geq 5\}$.

El supremo del intervalo $[0,1)\subseteq \mathbb{R}$ es $1$ ya que es el mínimo del conjunto de sus cotas superiores $\operatorname{CS}([0,1) = \{x:x\geq 1\}$.
:::
...
El ínfimo del intervalo $[0,1)\subseteq \mathbb{R}$ es $0$ ya que es el máximo del conjunto de sus cotas inferiores $\operatorname{CI}([0,1)) = \{x:x\leq 0\}$.
:::
